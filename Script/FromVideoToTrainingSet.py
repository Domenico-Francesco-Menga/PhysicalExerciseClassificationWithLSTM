...
for isFlip in range(0,2):
  ...
  ##########################################################################
  ####################### FOR PER IL TRAINING SET ##########################
  ##########################################################################

  for key in typeOfExerciseTraining:
    #verifica momentanea per vedere le righe dei file con il flip = 1
    if isFlip == 0:
      writeFile(key,"Target",["====================================================="])
      writeFile(key, "Training", ["====================================================="])
    for index in range(1, typeOfExerciseTraining[key] + 1):
      videoPath = r"C:\Users\menga\OneDrive\Desktop\VideoEsercizi"
      videoPath = videoPath + "\\" + key + "\\" + "training"+ "\\" + key + str(index) + ".mp4"
      print(videoPath)
      if isFlip == 0:
